...
# Allow COUNTRIES_FILE to be set via env var, but fall back to the repo's cleaned/countries.csv
if not COUNTRIES_FILE.exists():
    alt = CLEANED_DIR / 'countries.csv'
    alt2 = BASE_DIR / 'cleaned' / 'countries.csv'
    if alt.exists():
        safe_print(f"COUNTRIES_FILE not found at {COUNTRIES_FILE}, falling back to {alt}")
        COUNTRIES_FILE = alt
    elif alt2.exists():
