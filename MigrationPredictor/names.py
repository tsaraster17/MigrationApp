# migration_app_final_stack_dynamic_fixed.py
# Full integrated stacked migration predictor + dynamic macro↔migration coupling + Dash UI
# Save and run: python migration_app_final_stack_dynamic_fixed.py
...
MODEL_META_FILE = CACHE_DIR / "stacked_model_meta.pkl"   # <-- metadata (feature names) used to detect mismatch
...
def normalize_macro_df(df):
    ...
    Handles:
      - World Bank wide format (Country Code + year columns like '1960','1961'...)
      - OWID/long form with columns (Code/iso3, year, value)
      - Generic wide with first non-year column as country code/name
...
# These are preserved as real features (so the model learns any seasonal signal if present).
...
def save_model_and_meta(mdl, feature_list):
    with open(STACKED_MODEL_FILE, "wb") as f:
        pickle.dump(mdl, f)
    meta = {'feature_cols': list(feature_list)}
...
if stack_model is not None and model_meta is not None:
    ...
    # Compare sets and order-insensitive presence: if any expected feature in current feature_cols missing in trained_cols or vice versa, retrain
...
#    - Uses macro_future_preds baseline but applies adjustments driven by migration/pop changes
#    - Caches resulting quarterly predictions in QUARTERLY_PRED_FILE
# -------------------------
...
if regen:
    ...
    # We'll build dict: state[iso] = { 'population':..., 'gdp':..., 'gdp_percap':..., 'education':..., 'unemployment':..., 'employment_sector':... }
    state = {}
    ...
    for year in range(START_YEAR, END_YEAR+1):
        for qidx, quarter in enumerate(QUARTERS, start=1):
            ...
            Xf_cols = [c for c in feature_cols if c in df_future.columns] + macro_cols_this + ['quarter_sin','quarter_cos']
            ...
            if trained_feature_order is not None:
                ...
                for c in missing_in_Xf:
                    Xf[c] = 0.0
                # reindex (this will ensure column order matches training)
                Xf = Xf.reindex(columns=trained_feature_order, fill_value=0.0)
            else:
                # if we don't have meta, ensure we still pass the columns model expects (best-effort)
                # we can attempt to infer from model if it has attribute 'n_features_in_' but safer to proceed with current Xf
                pass
            ...
            # We'll compute net migration per country from df_future
            ...
            # Sensitivity hyperparams (tunable)
            ...
            for iso in state.keys():
                ...
                # adjust GDP by population change proportionally (simple heuristic)
                ...
                # recompute gdp_percap
                state[iso]['gdp_percap'] = (adjusted_gdp / new_pop) if new_pop>0 else 0.0
                # adjust other macros crudely: unemployment, education affected by migration/pop changes slightly
                if 'unemployment' in state[iso]:
                    ...
                    # if big inflow, unemployment may increase slightly; if outflow, decrease slightly
                ...
                if 'education' in state[iso]:
                    ...
                    state[iso]['education'] = base_edu  # keep baseline (or small drift could be applied)
                # employment_sector keep baseline if available
